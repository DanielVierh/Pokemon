...
const catchedPokemonContaier = document.getElementById(
  "catchedPokemonContaier",
);
...
const btn_refresh_todays_Pokemon = document.getElementById(
  "btn_refresh_todays_Pokemon",
);
...
const todayPokemonListWindow = document.getElementById(
...
const btnCancelMoveLearn = document.getElementById("btnCancelMoveLearn");
const btnCancelReplacePokemon = document.getElementById(
  "btnCancelReplacePokemon",
);
let addedPokemon;
...
function load_SaveObj() {
  if (localStorage.getItem("stored_save_Object") != null) {
    ...
    document.getElementById("outpCatchedAmount").innerHTML =
      `${myCatchedPokemons.length} gefangen`;
...
function renderCatchedPokemons() {
  catchedPokemonContaier.innerHTML = "";

  if (pokemonByID.length === 0) {
    const emptyState = document.createElement("p");
    emptyState.classList.add("emptyState");
    emptyState.innerHTML = "Noch keine Pokémon gefangen.";
    catchedPokemonContaier.appendChild(emptyState);
    return;
  }
  ...
  for (let i = 0; i < pokemonByID.length; i++) {
    ...
    addBtn.title = "Ins Team aufnehmen";
    addBtn.setAttribute("aria-label", "Ins Team aufnehmen");
    ...
    sellBtn.title = "Pokémon verkaufen";
    sellBtn.setAttribute("aria-label", "Pokémon verkaufen");
    sellBtn.setAttribute("tabindex", "0");
    ...
    pokename.innerHTML =
      makeFirstLetterBig(pokemonByID[i].name) + ` | Lv.${pokemonByID[i].level}`;
    ...
    button.addEventListener("click", () => {
      ...
      if (pokemonIndex >= 0) {
        ...
        if (freeTeamNumbers < 4) {
          ...
          document
            .getElementById("modal_Replace_Pokemon")
      ...
      if (myCatchedPokemons.length > 1) {
        ...
        if (pokemonIndex >= 0) {
          const pokemonPrice = parseInt(
            myCatchedPokemons[pokemonIndex].level * 9,
          );
          const confirmSell = window.confirm(
            `Möchtest du das Pokemon: "${myCatchedPokemons[pokemonIndex].name}" wirklich verkaufen? \nDu erhälst dafür ${pokemonPrice}$`,
          );
          if (confirmSell) {
...
if (detailBtn) {
  detailBtn.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (pokemonIndex >= 0) {
        console.log(
          "myCatchedPokemons[pokemonIndex]",
          myCatchedPokemons[pokemonIndex],
        );
        cont_Detail.classList.add("active");
        document.getElementById("det_Pokename").innerHTML = makeFirstLetterBig(
          myCatchedPokemons[pokemonIndex].name,
        );
        document.getElementById("det_PokemonImage").src =
          myCatchedPokemons[pokemonIndex].spriteFront;
        document.getElementById("det_Attack").innerHTML =
          `Angriff: ${myCatchedPokemons[pokemonIndex].statAttack}`;
        document.getElementById("det_Def").innerHTML =
          `Verteidigung: ${myCatchedPokemons[pokemonIndex].statDefense}`;
        document.getElementById("det_Lv").innerHTML =
          `Lv.${myCatchedPokemons[pokemonIndex].level}`;
        document.getElementById("det_HP").innerHTML =
          `Max KP: ${myCatchedPokemons[pokemonIndex].maxHp}`;
        document.getElementById("det_Type").innerHTML =
          `Typ: ${myCatchedPokemons[pokemonIndex].type}`;
        document.getElementById("det_Nr").innerHTML =
          `Nr: ${myCatchedPokemons[pokemonIndex].id}`;
        document.getElementById("det_catchDate").innerHTML =
          `Gefangen am: <br> -`;
        document.getElementById("det_catchName").innerHTML = `Name: <br> -`;
        document.getElementById("det_catchLv").innerHTML = `Level: <br> -`;
        try {
          document.getElementById("det_catchDate").innerHTML =
            `Gefangen am: <br> ${myCatchedPokemons[pokemonIndex].catchval.catchDate}`;
          document.getElementById("det_catchName").innerHTML =
            `Name: <br> ${makeFirstLetterBig(myCatchedPokemons[pokemonIndex].catchval.catchName)}`;
          document.getElementById("det_catchLv").innerHTML =
            `Level: <br> ${myCatchedPokemons[pokemonIndex].catchval.catchLevel}`;
        } catch (error) {}
          ...
          button.addEventListener("click", () => {
            if (index > 3) {
              if (is_LearningNewMove === false) {
                const confirmLerning = window.confirm(
                  `Möchtest du, dass dein Pokemon die Attacke ${myCatchedPokemons[currentDetailPokemonIndex].moves[index]} erlernt?`,
                );
                if (confirmLerning) {
                  ...
                  for (let i = 0; i <= 3; i++) {
                    document.getElementById(`mv_${i}`).innerHTML =
                      myCatchedPokemons[pokemonIndex].moves[i];
                  }
                }
              }
            }
...
if (btnCancelMoveLearn) {
  btnCancelMoveLearn.addEventListener("click", () => {
    is_LearningNewMove = false;
    modalMoves.classList.remove("active");
  });
}

if (btnCancelReplacePokemon) {
  btnCancelReplacePokemon.addEventListener("click", () => {
...
function learn_forget_Attack(forgotIndex) {
  ...
  const toForgetMoveName =
    myCatchedPokemons[currentDetailPokemonIndex].moves[forgotIndex];
  const toLearnMoveName =
    myCatchedPokemons[currentDetailPokemonIndex].moves[newLearnedMoveIndex];
  myCatchedPokemons[currentDetailPokemonIndex].moves.splice(
    forgotIndex,
    1,
    toLearnMoveName,
  );
  ...
  is_LearningNewMove = false;
  alert(
    `${makeFirstLetterBig(toLearnMoveName)} wurde erlernt und ${makeFirstLetterBig(toForgetMoveName)} wurde vergessen.`,
  );
  save_SaveObj();
  cont_Detail.classList.remove("active");
}
...
function renderTeam() {
  ...
  if (myTeam.length === 0) {
    const emptyState = document.createElement("p");
    emptyState.classList.add("emptyState");
    emptyState.innerHTML = "Team ist leer. Füge ein Pokémon mit ＋ hinzu.";
    teamPokemonContainer.appendChild(emptyState);
    return;
  }
  ...
  for (let i = 0; i < myTeam.length; i++) {
    ...
    delBtn.title = "Aus Team entfernen";
    delBtn.setAttribute("aria-label", "Aus Team entfernen");
    ...
    moveBtn.title = "In Team-Reihenfolge nach unten";
    moveBtn.setAttribute("aria-label", "In Team-Reihenfolge nach unten");
...
if (btn_ResetGame) {
  btn_ResetGame.addEventListener("click", () => {
    const decision = window.confirm(
      "Soll das Spiel wirklich zurückgesetzt werden? Der ganze Spielstand geht verloren!",
    );
    if (decision) {
      save_Object = {
        today_Date: "",
...
if (btn_refresh_todays_Pokemon) {
  btn_refresh_todays_Pokemon.addEventListener("click", () => {
    const request = window.confirm(
      "Sollen die Pokemon von Heute neu gemischt werden?",
    );
    if (request) {
      resetPokemon();
    }
