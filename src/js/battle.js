...
//######################################################
let isPvP = false;
let pvp_myTurn = false;
let pvp_socket = null;
let pvp_opponentName = "";
let pvp_opponentTeam = [];
let pvp_opponentIndex = 0;
let pvpLastDamageDealt = 0;
let pvpLastMoveName = "";
...
const trainer_battleCounter = document.getElementById(
...
function init() {
  ...
  if (document.getElementById("battleTag")) {
    ...
    // PvP-Modus prüfen
    const _pvpParams = new URLSearchParams(window.location.search);
    if (_pvpParams.get("pvp") === "true") {
    ...
    if (is_trainerBattle === false) {
      ...
      renderItems();
      document
        .getElementById("battleTag")
        .classList.add("trainer-battle-container");
      //trainerBattle_round++;
      document.getElementById("mainButton2").style.display = "none";
      ...
      try {
        if (is_trainerBattle === true) {
          trainerBattle_round = JSON.parse(
            localStorage.getItem("stored_battlecounter"),
          );
...
function createMyFirstPokemon() {
  ...
  try {
    ...
    if (choosenPokemon.isDefeated === false) {
      ...
      showInfoBox(
        `Los ${makeFirstLetterBig(choosenPokemon.name)}. Du schaffst das`,
      );
    } else {
      alert("Ein besiegtes Pokemon kann nicht in den Kampf geschickt werden");
    }
  } catch (error) {
    console.log(error);
  }
...
function createWildPokemon() {
  ...
  for (let i = 0; i < facedPokemons.length; i++) {
    if (randomPokemon === facedPokemons[i].id) {
      ...
      let four_moves = [];
      for (let i = 0; i <= 4; i++) {
        const randomMove = Math.floor(Math.random() * all_moves.length + 1);
        four_moves.push(all_moves[randomMove]);
      }
      if (facedPokemons[i].spriteBack === null) {
        console.log(
          "%c Pokemon hat KEIN alternatives BackSprite",
          "background-color: red; color: black; font-size: 2rem;",
        );
      }
  ...
  fetch(`https://pokeapi.co/api/v2/pokemon/${id}/`)
    ...
    .then((data) => {
      ...
      if (data.sprites.back_default === null) {
        console.log(
          "%c Pokemon hat kein Default BackSprite",
          "background-color: yellow; color: black; font-size: 2rem;",
        );
        if (data.sprites.other.showdown.back_default === null) {
          console.log(
            "%c Pokemon hat kein Default und kein Alternatives BackSprite",
            "background-color: red; color: black; font-size: 2rem;",
          );
        } else {
          console.log(
            "%c Pokemon hat alternativen BackSprite",
...
function myPokemonAttack(whoIsExecuting) {
  ...
  if (whoIsExecuting === "wildPokemon") {
    ...
    // PvP: Schaden und Attackenname für Socket-Übermittlung merken
    if (isPvP) {
      pvpLastDamageDealt = damage;
      pvpLastMoveName = pokeMove
        ? pokeMove.germanName || pokeMove.name || ""
        : "";
    }
    if (damage > 0) {
...
function animateProgressBar(damage, whoIsAffected, healVal, is_strike) {
  ...
  if (whoIsAffected === "myPokemon") {
    ...
    document.getElementById(`teamPokeProgress_${myCurrentPokemonIndex}`).value =
      hpInPercent;
  }
      ...
      setTimeout(() => {
        ...
        if (whoIsAffected === "myPokemon") {
          ...
          document.getElementById(
            `teamPokeProgress_${myCurrentPokemonIndex}`,
          ).value = hpInPercent;
          document
...
function checkWhoExecuteNext() {
  if (isPvP) {
    if (iamExecuting === false) {
      // Spieler hat gerade angegriffen → Angriff an Server senden, auf Gegner warten
      disableMainButtons();
      iamExecuting = true;
      myPkeBattleCard.classList.remove("active");
      wildPkeBattleCard.classList.add("active");
      const _overlay = document.getElementById("pvpWaitOverlay");
      if (_overlay) _overlay.style.display = "flex";
      if (pvp_socket) {
        pvp_socket.emit("pvp-attack", {
          moveName: pvpLastMoveName,
          damage: pvpLastDamageDealt,
          remainingHp: currentWildPokeHP,
          pokemonIndex: pvp_opponentIndex,
        });
      }
    }
    // iamExecuting === true → warten, bis der Gegner angreift (Socket-Event)
    return;
  }
...
function ki_Move() {
  if (isPvP) {
    // In PvP: KI greift nicht an – Züge kommen via Socket
    if (currentWildPokeHP <= 0) {
      pvpOpponentPokemonDefeated();
    }
    return;
  }
      ...
      if (is_trainerBattle === true && trainerBattle_round < 4) {
        ...
        if (trainerBattle_round === 4) {
          ...
          if (winner_random_number <= 3) {
            alert(
              "Klasse, du hast alle drei Trainer besiegt. Du gewinnst 5 Pokebälle",
            );
            save_Object.items.pokeballs += 5;
          } else if (winner_random_number >= 4 && winner_random_number <= 7) {
            alert(
              "Klasse, du hast alle drei Trainer besiegt. Du gewinnst 2 Supertränke",
            );
            save_Object.items.trank += 2;
          } else {
            alert(
              "Klasse, du hast alle drei Trainer besiegt. Du gewinnst 2 Beleber",
            );
          ...
          localStorage.setItem(
            "stored_battlecounter",
            JSON.stringify(trainerBattle_round),
          );
  ...
  if (myPokeballAmount > 0) {
    ...
    setTimeout(() => {
      ...
      if (catchquote >= 25) {
        ...
        showInfoBox(
          `${makeFirstLetterBig(currentWildPokemon.name)} wurde gefangen`,
        );
  ...
  pokemon1.addEventListener("click", () => {
    try {
      ...
      if (choosenPokemon.isDefeated === false) {
        ...
        if (isPvP && pvp_socket) {
          pvp_socket.emit("switch-pokemon", { pokemonIndex: 0 });
        } else {
          checkWhoExecuteNext();
        }
      } else {
  ...
  pokemon2.addEventListener("click", () => {
    try {
      ...
      if (choosenPokemon.isDefeated === false) {
        ...
        if (isPvP && pvp_socket) {
          pvp_socket.emit("switch-pokemon", { pokemonIndex: 1 });
        } else {
          checkWhoExecuteNext();
        }
      } else {
  ...
  pokemon3.addEventListener("click", () => {
    try {
      ...
      if (choosenPokemon.isDefeated === false) {
        ...
        if (isPvP && pvp_socket) {
          pvp_socket.emit("switch-pokemon", { pokemonIndex: 2 });
        } else {
          checkWhoExecuteNext();
        }
      } else {
  ...
  pokemon4.addEventListener("click", () => {
    try {
      ...
      if (choosenPokemon.isDefeated === false) {
        ...
        if (isPvP && pvp_socket) {
          pvp_socket.emit("switch-pokemon", { pokemonIndex: 3 });
        } else {
          checkWhoExecuteNext();
        }
      } else {
...
function level_up() {
  ...
  const calcXP = Math.floor(
    (enemyLevel * wildPokemon_MaxHP * (fightRate + 2)) / currentLevel,
  );
  console.warn(
    `eLv(${enemyLevel}) * wPMHP(${wildPokemon_MaxHP}) / CLv(${currentLevel}) = ${calcXP}`,
  );
  const newXP = oldXP + calcXP;
  let pokemonIndex = -1;
  for (let i = 0; i < save_Object.myCatchedPokemons.length; i++) {
    if (
      myStaticPokemon.unique_ID === save_Object.myCatchedPokemons[i].unique_ID
    ) {
      pokemonIndex = i;
...
function levelUp_with_Evolving(currentLevel, pokemonIndex) {
  ...
  fetch(
    `https://pokeapi.co/api/v2/pokemon-species/${makeFirstLetterSmall(myStaticPokemon.name)}/`,
  )
    .then((res) => res.json())
    .then((data) => {
      fetch(data.evolution_chain.url)
        ...
        .then((data2) => {
          ...
          try {
            fetchNewPokemonUrl =
              data2.chain.evolves_to[0].evolves_to[0].species.url;
          } catch (error) {}
              ...
              fetch(`https://pokeapi.co/api/v2/pokemon/${evolveToId}/`)
                ...
                .then((data4) => {
                  ...
                  if (oldID !== evolveToId) {
                    ...
                    ].spriteBack = evolve_spriteBack;
                    save_Object.myPokemonTeam[
                      myCurrentPokemonIndex
                    ].spriteFront = evolve_spriteFront;
                    save_Object.myPokemonTeam[myCurrentPokemonIndex].id =
                      evolveToId;
                    save_Object.myPokemonTeam[myCurrentPokemonIndex].name =
                      evolvedName;
                    save_Object.myCatchedPokemons[pokemonIndex].spriteBack =
                      evolve_spriteBack;
                    save_Object.myCatchedPokemons[pokemonIndex].spriteFront =
                      evolve_spriteFront;
                    save_Object.myCatchedPokemons[pokemonIndex].id = evolveToId;
                    save_Object.myCatchedPokemons[pokemonIndex].name =
                      evolvedName;
                    ...
                    save_Object.myCatchedPokemons[pokemonIndex].hp += 1;
                    save_Object.myCatchedPokemons[pokemonIndex].statAttack += 1;
                    save_Object.myCatchedPokemons[pokemonIndex].statDefense +=
                      1;
                    myPokemonXPProgress.value = myStaticPokemon.xp;
                    save_SaveObj();
                    alert(
                      `Wow, ${makeFirstLetterBig(myStaticPokemon.name)} entwickelt sich zu ${evolvedName}`,
                    );
                    showInfoBox(`${makeFirstLetterBig(myStaticPokemon.name)} erreicht Level ${currentLevel} <br>
                                            KP: + 1 <br>
                                            Angriff: + 1 <br>
...
function renderItems() {
  ...
  document.getElementById("beleberAmount").innerHTML =
    save_Object.items.beleber;
...
if (giveTrank) {
  giveTrank.addEventListener("click", () => {
    ...
    if (save_Object.items.trank >= 1) {
      ...
      const healRequest = window.confirm(
        `Möchtest du Deinem Pokemon ${makeFirstLetterBig(pokemonName)} \n 1 Supertrank geben?`,
      );
      if (healRequest) {
        ...
        if (staticPokemonUniqueId === itemPokemonUniqueId) {
          ...
          document.getElementById(`teamPokeProgress_${pokemonItemId}`).value =
            hpPerc;
...
if (giveBeleber) {
  giveBeleber.addEventListener("click", () => {
    ...
    if (save_Object.items.beleber >= 1) {
      const pokemonName = myTeam[pokemonItemId].name;
      const healRequest = window.confirm(
        `Möchtest du Dein Pokemon ${makeFirstLetterBig(pokemonName)} \n wiederbeleben?`,
      );
      if (healRequest) {
        //* Herausfinden, ob Pokemon gerade kämpft
        const staticPokemonUniqueId = myStaticPokemon.unique_ID;
        ...
        if (staticPokemonUniqueId === itemPokemonUniqueId) {
          ...
          document
            .getElementById(`teamPoke_${pokemonItemId}`)
            .classList.remove("defeat");
...
function check_if_all_defeated() {
  ...
  if (minimum_one_alive === false) {
    if (isPvP) {
      // Im PvP verloren
      showInfoBox("Alle deine Pokemon wurden besiegt!");
      if (pvp_socket) {
        pvp_socket.emit("battle-end", { winner: "opponent" });
      }
      setTimeout(() => {
        pvpHandleBattleEnd("opponent");
      }, 2000);
      return;
    }
...
//######################################################
//*ANCHOR - PvP Funktionen
//######################################################
function initPvPMode() {
  pvp_opponentName = sessionStorage.getItem("pvp_opponentName") || "Gegner";
  pvp_opponentTeam = JSON.parse(
    sessionStorage.getItem("pvp_opponentTeam") || "[]",
  );
  pvp_myTurn = sessionStorage.getItem("pvp_myTurn") === "1";
  const _roomCode = sessionStorage.getItem("pvp_roomCode");
  const _myName = sessionStorage.getItem("pvp_trainerName") || "Trainer";

  if (!pvp_opponentTeam.length || !_roomCode) {
    alert("PvP-Daten fehlen. Zurück zur Lobby.");
    window.location = "pvp-lobby.html";
    return;
  }

  // Gegner-Trainer-Name einblenden
  const _opponentLabel = document.getElementById("pvpOpponentName");
  if (_opponentLabel) {
    _opponentLabel.style.display = "block";
    _opponentLabel.textContent = `⚔️ ${pvp_opponentName}`;
  }

  // Erstes Gegner-Pokemon laden
  pvpLoadOpponentPokemon(0);

  // Buttons initial entsprechend dem Startzug setzen
  if (pvp_myTurn) {
    iamExecuting = false;
    enableMainButtons();
  } else {
    iamExecuting = true;
    disableMainButtons();
    const _ov = document.getElementById("pvpWaitOverlay");
    if (_ov) _ov.style.display = "flex";
  }

  // Socket verbinden und Raum wieder beitreten
  pvp_socket = io();
  pvp_socket.on("connect", () => {
    pvp_socket.emit("rejoin-battle", {
      roomCode: _roomCode,
      trainerName: _myName,
    });
  });

  pvp_socket.on("battle-ready", ({ myTurn }) => {
    pvp_myTurn = myTurn;
    if (myTurn) {
      iamExecuting = false;
      enableMainButtons();
  ...
  pvp_socket.on("opponent-attacked", ({ moveName, damage, remainingHp }) => {
    pvpReceiveOpponentAttack(moveName, damage, remainingHp);
  });

  pvp_socket.on("opponent-switched", ({ pokemonIndex }) => {
    pvpLoadOpponentPokemon(pokemonIndex);
    showInfoBox(
      `${pvp_opponentName} schickt ${pvp_opponentTeam[pokemonIndex]?.name || "ein Pokemon"} in den Kampf!`,
    );
    // Der Angreifer (ich) darf jetzt angreifen
    iamExecuting = false;
    enableMainButtons();
    const _ov = document.getElementById("pvpWaitOverlay");
    if (_ov) _ov.style.display = "none";
  ...
  pvp_socket.on("wait-for-attack", () => {
    // Ich habe gewechselt, Gegner greift jetzt an
    iamExecuting = true;
    disableMainButtons();
    const _ov = document.getElementById("pvpWaitOverlay");
    if (_ov) {
      _ov.style.display = "flex";
      const _txt = document.getElementById("pvpWaitText");
      if (_txt) _txt.textContent = `${pvp_opponentName} greift an…`;
    }
  });

  pvp_socket.on("battle-end", ({ winner }) => {
    pvpHandleBattleEnd(winner);
  });

  pvp_socket.on("turn-granted", () => {
    // Mein Angriff wurde verarbeitet, Gegner ist jetzt am Zug
    // (kein UI-Update nötig – das passiert schon in checkWhoExecuteNext)
  });

  pvp_socket.on("opponent-disconnected", () => {
    alert(`${pvp_opponentName} hat die Verbindung getrennt. Du gewinnst!`);
    save_Object.items.money += 50;
    save_SaveObj();
    pvpCleanup();
    window.location = "map1.html";
  });

  pvp_socket.on("connect_error", () => {
    alert("Verbindung zum PvP-Server verloren. Zurück zur Lobby.");
    window.location = "pvp-lobby.html";
  });
}

function pvpLoadOpponentPokemon(index) {
  pvp_opponentIndex = index;
  const p = pvp_opponentTeam[index];
  if (!p) return;
  ...
  currentWildPokeHP = p.hp || p.maxHp;

  if (wildPokeImage) wildPokeImage.src = p.spriteFront || "";
  if (wildPokeImage) wildPokeImage.style.opacity = "1";
  if (wildPokeImage) wildPokeImage.classList.remove("getDestroyed");
  if (wildPokeName)
    wildPokeName.innerHTML = `${makeFirstLetterBig(p.name)} | Lv. ${p.level} | KP.${currentWildPokeHP}`;
  if (wildPokemonProgress) wildPokemonProgress.value = 100;
}

function pvpReceiveOpponentAttack(moveName, damage, remainingHp) {
  myCurrentPokemonHP = remainingHp;
  ...
  showInfoBox(
    `${pvp_opponentName} setzt "${moveName}" ein und richtet ${damage} Schaden an.`,
  );

  const hpPercent = Math.max(
    0,
    parseInt((remainingHp * 100) / myCurrentPokemonStaticHP),
  );
  myPokemonProgress.value = hpPercent;
  try {
    document.getElementById(`teamPokeProgress_${myCurrentPokemonIndex}`).value =
      hpPercent;
  } catch (e) {}
  ...
  setTimeout(() => {
    if (remainingHp <= 0) {
      ...
      myPokeName.innerHTML = `${makeFirstLetterBig(myStaticPokemon.name)} | Lv. ${myStaticPokemon.level} | KP.${remainingHp}`;
      // Mein Zug
      iamExecuting = false;
      enableMainButtons();
      const _ov = document.getElementById("pvpWaitOverlay");
...
function pvpOpponentPokemonDefeated() {
  const nextIndex = pvp_opponentIndex + 1;
  if (nextIndex < pvp_opponentTeam.length) {
    // Gegner hat noch Pokemon → Gegner wechselt (kommt via opponent-switched Event)
    showInfoBox(
      `${pvp_opponentName}s Pokemon wurde besiegt! Warte auf nächstes Pokemon…`,
    );
    disableMainButtons();
    const _ov = document.getElementById("pvpWaitOverlay");
    if (_ov) {
      _ov.style.display = "flex";
      document.getElementById("pvpWaitText").textContent =
        `${pvp_opponentName} wechselt Pokemon…`;
    }
  } else {
    // Alle Gegner-Pokemon besiegt → Sieg
    showInfoBox(
      `Alle Pokemon von ${pvp_opponentName} wurden besiegt! Du gewinnst!`,
    );
    if (pvp_socket) {
      pvp_socket.emit("battle-end", { winner: "me" });
    }
    setTimeout(() => {
      pvpHandleBattleEnd("me");
    }, 2000);
  }
}

function pvpHandleBattleEnd(winner) {
  if (winner === "me") {
    save_Object.items.money += 50;
    save_SaveObj();
    alert("Du hast den PvP-Kampf gewonnen! +50 Münzen");
  } else {
    alert("Du hast den PvP-Kampf verloren!");
  }
  pvpCleanup();
  window.location = "map1.html";
}
...
function pvpCleanup() {
  if (pvp_socket) {
    pvp_socket.disconnect();
    pvp_socket = null;
  }
  sessionStorage.removeItem("pvp_trainerName");
