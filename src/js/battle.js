...
//*ANCHOR - Fetch Attack
//######################################################
async function fetchAttack(nameId) {
  const response = await fetch(`https://pokeapi.co/api/v2/move/${nameId}/`);
  const data = await response.json();
  console.log("Move", data);

  const germanMoveName =
    data.names?.find((nameItem) => nameItem.language?.name === "de")?.name ??
    data.name;
  pokeMove = new PokeMove(
    ...
    data.accuracy ?? 100,
    data.power ?? 0,
    data.meta?.min_hits ?? 1,
    data.meta?.max_hits ?? 1,
    data.pp ?? 0,
  ...
  for (let mvI = 0; mvI < allMoves.length; mvI++) {
    if (allMoves[mvI].name === pokeMove.name) {
  ...
  if (move_in_AllPomkemonMoves === false) {
    ...
    if (allMoves !== save_Object.allPokemonMoves) {
...
//*ANCHOR - Init Move
//######################################################
async function init_Move(moveName) {
...
function calculateBattleDamage(
  ...
  const power = baseDamage ?? 0;
  if (power <= 0) {
    return 0;
  }

  const stab = attackerType === attackType ? 1.5 : 1;
  const randomFactor = (85 + parseInt(Math.random() * 16)) / 100;
  const rawDamage =
    (((2 * attackerLevel) / 5 + 2) *
      power *
      (attackVal / Math.max(1, defenceVal))) /
      50 +
    2;

  return Math.max(1, parseInt(rawDamage * stab * typeCalc * randomFactor));
}
...
function myPokemonAttack(whoIsExecuting) {
  ...
  let damage = calculateBattleDamage(
    lv,
    whoIsExecuting === "wildPokemon"
      ? currentWildPokemon.type
      : myStaticPokemon.type,
    attackType,
    attackVal,
    defenceVal,
    attbaseDamage,
  ...
  if (!didMoveHit(pokeMove.accuracy)) {
    damage = 0;
    is_strike = false;
  }
...
function chooseNewPokemon(choosenPokemon) {
  ...
  updateXpProgressBar(myStaticPokemon);
...
function updateXpProgressBar(pokemon) {
  if (!myPokemonXPProgress || !pokemon) {
    return;
  }

  myPokemonXPProgress.max = getXpToNextLevel(pokemon.level);
  myPokemonXPProgress.value = Math.min(pokemon.xp, myPokemonXPProgress.max);
}
...
function levelUp_with_learn_Attack(currentLevel, pokemonIndex) {
  ...
  try {
    ...
    if (
      newAttackName !== undefined &&
      save_Object.myCatchedPokemons[pokemonIndex].moves.includes(
        newAttackName,
      ) === false
    ) {
      if (save_Object.myCatchedPokemons[pokemonIndex].moves.length >= 4) {
        const replacedMove =
          save_Object.myCatchedPokemons[pokemonIndex].moves[3];
        save_Object.myCatchedPokemons[pokemonIndex].moves[3] = newAttackName;
        newAttackMessage = `${makeFirstLetterBig(
          myStaticPokemon.name,
        )} ersetzt ${replacedMove} durch ${newAttackName}`;
      } else {
  ...
  save_Object.myPokemonTeam[myCurrentPokemonIndex].moves = [
